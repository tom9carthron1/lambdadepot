...
/**
 * Success {@link Result} implementation.
 *
 * <p>Success {@link Result}s have a value and no error.
 *
 * @param <T> value type
 */
public final class ResultSuccess<T> extends Result<T> {
    ...
    public <R> Result<R> map(Function<T, R> mapper) {
        ...
        try {
            return successOrEmpty(mapper.apply(value));
        } catch (Exception e) {
            return failure(e);
        }
    }
     ...
     *
     * <p>Note: A method reference to the exception constructor with an empty
     * argment list can be used as the supplier. For example, {@code IllegalStateException::new}.
     *
