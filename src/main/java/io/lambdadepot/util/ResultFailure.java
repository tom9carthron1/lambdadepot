...
/**
 * Failure {@link Result} implementation.
 *
 * <p>Failure {@link Result}s have an error and no value.
 *
 * @param <T> value type
 */
...
public final class ResultFailure<T> extends Result<T> {
    ...
    public <E extends Throwable> Result<T> ifFailureMap(Class<E> exceptionType, Function<? super E, ? extends Throwable> mapper) {
        ...
        Objects.requireNonNull(mapper, "mapper");
        return exceptionType.isInstance(error) ? failure(mapper.apply(exceptionType.cast(error))) : this;
    }
    ...
    public Result<T> ifFailureMap(Predicate<? super Throwable> predicate, Function<? super Throwable, ? extends Throwable> mapper) {
        Objects.requireNonNull(predicate, "predicate");
        Objects.requireNonNull(mapper, "mapper");
        return predicate.test(error) ? failure(mapper.apply(error)) : this;
    }
    ...
    /**
     * <p>Return the contained value if instance of {@code ResultSuccess},
     ...
     * error will be wrapped in a {@code RuntimeException} and then thrown.</p>
     *
     ...
     */
    @Override
    public <X1 extends Throwable, X2 extends Throwable> T orElseThrow(Supplier<? extends X1> emptySupplier, Supplier<? extends X2> failureSupplier) throws X1, X2 {
